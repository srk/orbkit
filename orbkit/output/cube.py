...
def cube_creator(data,filename,geo_info,geo_spec,comments='',labels=None,**kwargs):
  ...
  **Parameters:**
  ...
  if labels is not None:
    ...
    string += '\n'
  # Write data
  field_precision=kwargs.get('field_precision',5)
  if field_precision < 5:
      field_precision = 5
  #fieldformat = '{{0>:{0}.{1}E}}'.format(field_precision+8,field_precision)
  #print(fieldformat)
...
#          string += ('%(data).5E' % {'data': dd}).rjust(13)
          string += '{value:{fieldwidth}.{precision}E}'.format(value=dd,fieldwidth=field_precision+8,precision=field_precision)
          if (c % 6 == 5): 
            string += '\n'
          c += 1
      string += '\n'
...
def cube_creator2(data,filename,geo_info,geo_spec,comments='',labels=None,**kwargs):
  '''Creates a plain text Gaussian cube file. 
